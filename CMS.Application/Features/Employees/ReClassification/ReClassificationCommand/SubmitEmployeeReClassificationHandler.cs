...
{
    public class SubmitEmployeeReClassificationHandler:IRequestHandler<SubmitEmployeeReClassification, int>
    {
        private readonly IDataService dataService;

        public SubmitEmployeeReClassificationHandler(IDataService dataService)
        {
            this.dataService = dataService;
        }
        public async Task<int> Handle(SubmitEmployeeReClassification command,CancellationToken cancellationToken)
        {
            var submitted = await dataService.EmployeeReClassifications.FindAsync(command.Id);
