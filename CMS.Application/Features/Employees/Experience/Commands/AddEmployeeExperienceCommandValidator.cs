...
namespace CMS.Application.Features.Employees.Experience.Commands
{
    public class AddEmployeeExperienceCommandValidator : AbstractValidator<AddEmployeeExperienceCommand>
    {
        ...
        private bool BeUniqueExperience(AddEmployeeExperienceCommand command)
        {
            return !dataService.EmployeeExperiences
                .Any(exp => exp.StartDate == command.StartDate && exp.EndDate == command.EndDate && exp.EmployeeId==command.EmployeeId);
        }

        private bool BeMoreThanSixMonths(AddEmployeeExperienceCommand command)
        {
