...
export const RegionRejectedApprovalRequest = () => {
  ...
  const rejectedRegions = paginatedRegionsData?.items || [];

  // Filter regions based on searchQuery
  const filteredRejectedRegions = searchQuery
    ? rejectedRegions.filter(
        (region) =>
  ...
  const showNoMatchingResultsAlert =
    searchQuery && filteredRejectedRegions.length === 0;
        ...
        <Paper>
          <TableContainer>
            <Table size="medium">
              ...
              <TableBody>
                {filteredRejectedRegions.map((item: RegionDto) => (
