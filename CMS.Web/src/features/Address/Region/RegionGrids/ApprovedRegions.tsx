...
export const ApprovedRegions = () => {
  ...
  const approvedRegions = paginatedRegionsData?.items || [];

  // Filter regions based on searchQuery
  const filteredApprovedRegions = searchQuery
  ...
  const showNoMatchingResultsAlert =
    searchQuery && filteredApprovedRegions.length === 0;
    ...
    <Box>
        ...
        <Paper>
          <TableContainer>
            <Table size="medium">
              ...
              <TableBody>
                {filteredApprovedRegions.map((item: RegionDto) => (
      ...
      {showNoMatchingResultsAlert && (
        <Alert severity="info" sx={{ m: 2 }}>
          No matching Approved regions found
        </Alert>
      )}
      <Pagination
      ...
      {!isLoading && !counts?.approved && (
        <Box sx={{ p: 2, display: "flex", justifyContent: "center" }}>
          <Typography> No Approved Regions Available</Typography>
        </Box>
      )}
    </Box>
  );
