...
export const SubCityRejectedApprovalRequest = () => {
  ...
  const rejectedSubCities = subCitiesData?.items || [];

  // Filter sub-cities based on searchQuery
  const filteredRejectedSubCities = searchQuery
    ? rejectedSubCities.filter(
        (subCity) =>
  ...
  const showNoMatchingResultsAlert =
    searchQuery && filteredRejectedSubCities.length === 0;
        ...
        <Paper>
          <TableContainer>
            <Table size="medium">
              ...
              <TableBody>
                {filteredRejectedSubCities.map((item: SubCityDto) => (
