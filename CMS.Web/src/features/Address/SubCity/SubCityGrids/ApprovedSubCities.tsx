...
export const ApprovedSubCities = () => {
  ...
  const approvedSubCities = subCitiesData?.items || [];

  // Filter sub-cities based on searchQuery
  const filteredApprovedSubCities = searchQuery
    ? approvedSubCities.filter(
        (subCity) =>
  ...
  const showNoMatchingResultsAlert =
    searchQuery && filteredApprovedSubCities.length === 0;
        ...
        <Paper>
          <TableContainer>
            <Table size="medium">
              ...
              <TableBody>
                {filteredApprovedSubCities.map((item: SubCityDto) => (
