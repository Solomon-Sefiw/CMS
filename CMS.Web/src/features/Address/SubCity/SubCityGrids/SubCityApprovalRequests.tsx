...
export const SubCityApprovalRequests = () => {
  ...
  const submittedSubCities = subCitiesData?.items || [];

  // Filter sub-cities based on searchQuery
  const filteredSubmittedSubCities = searchQuery
    ? submittedSubCities.filter(
        (subCity) =>
  ...
  const showNoMatchingResultsAlert =
    searchQuery && filteredSubmittedSubCities.length === 0;
        ...
        <Paper>
          <TableContainer>
            <Table size="medium">
              ...
              <TableBody>
                {filteredSubmittedSubCities.map((item: SubCityDto) => (
