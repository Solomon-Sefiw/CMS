...
export const JobRoleCategoryApprovalRequests = () => {
  ...
  const submittedJobRoleCategories = jobRoleCategoriesData?.items || [];

  // Filter job role categories based on searchQuery
  const filteredSubmittedJobRoleCategories = searchQuery
    ? submittedJobRoleCategories.filter(
        (jobRoleCategory) =>
    ...
    : submittedJobRoleCategories;

  // Show "No Matching Results" alert
  const showNoMatchingResultsAlert =
    searchQuery && filteredSubmittedJobRoleCategories.length === 0;
    ...
    <Box>
        ...
        <Paper>
          <TableContainer>
            <Table size="medium">
              ...
              <TableBody>
                {filteredSubmittedJobRoleCategories.map(
      ...
      {!isDataLoading && !statusCounts?.submitted && (
        <Box sx={{ p: 2, display: "flex", justifyContent: "center" }}>
          <Typography>No Submitted job role categories available</Typography>
        </Box>
      )}
    </Box>
  );
