...
export const JobRoleCategoryRejected = () => {
  ...
  const rejectedJobRoleCategories = jobRoleCategoriesData?.items || [];

  // Filter job role categories based on searchQuery
  const filteredRejectedJobRoleCategories = searchQuery
    ? rejectedJobRoleCategories.filter(
        (jobRoleCategory) =>
    ...
    : rejectedJobRoleCategories;

  // Show "No Matching Results" alert
  const showNoMatchingResultsAlert =
    searchQuery && filteredRejectedJobRoleCategories.length === 0;
  ...
  return (
    <Box>
        ...
        <Paper>
          <TableContainer>
            <Table size="medium">
              ...
              <TableBody>
                {filteredRejectedJobRoleCategories.map(
      ...
      <Pagination
        ...
        totalRowsCount={statusCounts?.rejected}
      ...
      {!isDataLoading && !statusCounts?.rejected && (
        <Box sx={{ p: 2, display: "flex", justifyContent: "center" }}>
          <Typography>No rejected job role categories available</Typography>
        </Box>
      )}
    </Box>
  );
