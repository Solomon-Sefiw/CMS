...
export const RejectedAwards = () => {
  ...
  const rejectedAwards = awardsData?.items || []; // Changed variable name

  // Filter awards based on searchQuery
  const filteredRejectedAwards = searchQuery
    ? rejectedAwards.filter(
        (
    ...
    : rejectedAwards; // Changed variable name

  // Show "No Matching Results" alert
  const showNoMatchingResultsAlert =
    searchQuery && filteredRejectedAwards.length === 0; // Changed variable name
