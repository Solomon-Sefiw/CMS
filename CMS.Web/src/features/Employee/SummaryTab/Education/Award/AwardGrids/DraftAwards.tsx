...
export const DraftAwards = () => {
  ...
  const draftAwards = awardsData?.items || []; // Changed variable name

  // Filter awards based on searchQuery
  const filteredDraftAwards = searchQuery
    ? draftAwards.filter(
        (
    ...
    : draftAwards; // Changed variable name

  // Show "No Matching Results" alert
  const showNoMatchingResultsAlert =
    searchQuery && filteredDraftAwards.length === 0; // Changed variable name
