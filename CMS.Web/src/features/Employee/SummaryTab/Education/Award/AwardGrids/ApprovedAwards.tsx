...
export const ApprovedAwards = () => {
  ...
  const approvedAwards = awardsData?.items || []; // Changed variable name

  // Filter awards based on searchQuery
  const filteredApprovedAwards = searchQuery
    ? approvedAwards.filter(
        (
    ...
    : approvedAwards; // Changed variable name

  // Show "No Matching Results" alert
  const showNoMatchingResultsAlert =
    searchQuery && filteredApprovedAwards.length === 0; // Changed variable name
