...
export const ApprovedFieldOfStudies = () => {
  ...
  const approvedFieldOfStudies = fieldOfStudiesData?.items || [];

  const filteredApprovedFieldOfStudies = searchQuery
    ? approvedFieldOfStudies.filter(
        (fieldOfStudy) =>
          fieldOfStudy.name
            ?.toLowerCase()
  ...
  const showNoMatchingResultsAlert =
    searchQuery && filteredApprovedFieldOfStudies.length === 0;
        ...
        <Paper>
          <TableContainer>
            <Table size="medium">
              ...
              <TableBody>
                {filteredApprovedFieldOfStudies.map(
