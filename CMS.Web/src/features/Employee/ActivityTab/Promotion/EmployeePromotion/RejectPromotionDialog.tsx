...
import { useRejectEmployeePromotionMutation } from "../../../../../app/api";
import { RejectEmployeePromotion } from "../../../../../app/api";
...
export const RejectPromotionDialog: React.FC<RejectPromotionDialogProps> = ({
  Id,
  employeeId,
  onClose,
}) => {
  ...
  const [
    RejectingEmployeePromotion,
    { error: rejectingEmployeePromotionError },
...
const permissions = usePermission();
  const handleSubmit = async (
    ...
    try {
      ...
      await RejectingEmployeePromotion(payload).unwrap();
      onClose();
    } catch (error: any) {
      setStatus({
        success: false,
    ...
    <Dialog scroll="paper" disableEscapeKeyDown maxWidth="md" open>
      ...
      >
        {({ values, handleChange, touched, errors, isSubmitting, status }) => (
          <Form>
            <DialogHeader title="Reject Employee Promotion" onClose={onClose} />
