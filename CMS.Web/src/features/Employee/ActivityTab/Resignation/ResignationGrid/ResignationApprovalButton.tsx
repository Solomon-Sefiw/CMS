...
import { useSubmitResignationMutation } from "../../../../../app/store";
import { removeEmptyFields } from "../../../../../utils";
import { WorkflowActionDialog } from "../../../../../components";

export const ResignationApprovalButton = ({ id }: { id: number }) => {
  const permissions = usePermission();
  const [dialogOpened, setDialogOpened] = useState(false);
  const [submit, { error: submitError, reset: submitReset }] = useSubmitResignationMutation();
  ...
  const handleSubmit = useCallback((comment: string) => {
    submit({ submitResignationCommand: removeEmptyFields({ id, comment }) }).unwrap().then(onDialogClose).catch(() => {});
