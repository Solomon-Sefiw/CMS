...
interface SubmitReClassificationDialogProps {
  Id: number | undefined;
...
export const SubmitReClassificationDialog: React.FC<
  SubmitReClassificationDialogProps
> = ({ Id, employeeId, onClose }) => {
  ...
  const [
    SubmittingEmployeeReClassification,
    { error: submitEmployeeReClassificationerror },
  ] = useSubmittEmployeeReClassificationMutation();
  const handleSubmit = async (
    ...
    try {
      const payload = {
        submitEmployeeReClassification: {
          id: Id!,
      ...
      await SubmittingEmployeeReClassification(payload).unwrap();
      onClose();
    } catch (error: any) {
      console.error("Error Submition ReClassification:", error);
      setStatus({
        success: false,
    ...
    <Dialog scroll="paper" disableEscapeKeyDown maxWidth="md" open>
      ...
      >
        {({ values, handleChange, touched, errors, isSubmitting, status }) => (
          <Form>
            <DialogHeader
              title="Submit Employee ReClassification"
              onClose={onClose}
            />
