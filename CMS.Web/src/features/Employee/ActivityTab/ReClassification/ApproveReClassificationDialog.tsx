...
import { useApproveEmployeeReClassificationMutation } from "../../../../app/api/HCMSApi";
import { usePermission } from "../../../../hooks";
interface ApproveReClassificationDialogProps {
  Id: number | undefined;
...
export const ApproveReClassificationDialog: React.FC<
  ApproveReClassificationDialogProps
> = ({ Id, employeeId, onClose }) => {
  ...
  const [
    ApprovingEmployeeReClassification,
    { error: approveEmployeeReClassificationerror },
  ...
  const handleSubmit = async (
    ...
    try {
      const payload = {
        approveEmployeeReClassification: {
          id: Id!,
      ...
      await ApprovingEmployeeReClassification(payload).unwrap();
      onClose();
    } catch (error: any) {
      setStatus({
        success: false,
    ...
    <Dialog scroll="paper" disableEscapeKeyDown maxWidth="md" open>
      ...
      >
        {({ values, handleChange, touched, errors, isSubmitting, status }) => (
          <Form>
            <DialogHeader
              title="Approve Employee ReClassifcation"
              onClose={onClose}
            />
