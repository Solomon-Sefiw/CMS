...
//Demotion
export const EmployeeDemotionDialog = ({
  onClose,
}: {
  onClose: () => void;
}) => {
  const { showSuccessAlert, showErrorAlert } = useAlert();
  const [EmployeeDemotionData, setEmployeeDemotionData] =
    useState<AddEmployeeDemotionCommand>();
  const [addEmployeeDemotion, { error: AddingDemotionError }] =
    useAddEmployeeDemotionMutation();
  ...
  useEffect(() => {
    if (
      !EmployeeDemotionData &&
      employeeId &&
      employeeInfo &&
      employeeInfo.businessUnitID
    ) {
      setEmployeeDemotionData({
  ...
  }, [EmployeeDemotionData, employeeId, employeeInfo]);
  ...
  const handleSubmit = useCallback(
    (values: AddEmployeeDemotionCommand) => {
      ...
      addEmployeeDemotion({
        addEmployeeDemotionCommand: payload, // <- update this key
      })
        .unwrap()
        .then((response: any) => {
          showSuccessAlert("Employee Promotion Added Successfully");
          setEmployeeDemotionData(response);
          onClose();
        })
        .catch((error: any) => {});
    },
    [onClose, addEmployeeDemotion]
  );
  //
  ...
  const validationSchema = Yup.object({
    ...
    demotionDate: Yup.date()
      .required("demotion Date is required")
  ...
  //
  const errorsAddingDemotion = (AddingDemotionError as any)?.data?.errors;
  console.log(errorsAddingDemotion + "errorsAddingDemotion");
  return (
    <Dialog
      scroll={"paper"}
    ...
    >
      {!!EmployeeDemotionData && (
        <Formik
          initialValues={EmployeeDemotionData || emptyEmployeDemotion}
        ...
        >
          {({
            ...
            return (
              <Form>
                <DialogHeader title="Add Employee Demotion" onClose={onClose} />
                <DialogContent dividers>
                  <Grid container spacing={2}>
                    {errorsAddingDemotion && (
                      <Grid item xs={12}>
                        <Errors errors={errorsAddingDemotion as any} />
                      </Grid>
                    )}
                    <Grid item xs={12}>
