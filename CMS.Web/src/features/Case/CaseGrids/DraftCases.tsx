...
import { CaseDto, useGetAllCaseListsQuery, useGetCaseCountPerApprovalStatusQuery } from "../../../app/api/HCMSApi";
...
export const DraftCases = () => {
  ...
  const { data: itemsResponse, isLoading: isListLoading } = useGetAllCaseListsQuery({
...
console.log(itemsResponse)
  const { data: counts } = useGetCaseCountPerApprovalStatusQuery();
                ...
                <Table size="medium">
                  ...
                  <TableBody>
                    {sortedItems.map((item: CaseDto) => (
                      ...
                      >
                       <TableCell>{item.accusedName}</TableCell>
                                               <TableCell>{item.plaintiffName}</TableCell>
                                               <TableCell>{item.assignedJudge?.firstName}</TableCell>
                                               <TableCell>{item.caseNumber}</TableCell>
                                               <TableCell>{dayjs(item.filedAt).format("DD MMM YYYY")}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
